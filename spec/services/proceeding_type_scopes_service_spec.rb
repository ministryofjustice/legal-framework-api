  ...
  context "when the request is for PBM32A" do
    ...
    context "and no sgo_update parameter is sent in the payload" do
      it "returns the legacy set of scope limitations" do
        expect(proceeding_type_scopes_response[:level_of_service][:scope_limitations].count).to eq 24 # 25 on new ne
        expect(legacy_scope_limitation.count).to eq 4 # 4 keys in the object hash
    ...
    context "and an sgo_update parameter is sent as true in the payload" do
      ...
      it "returns the legacy set of scope limitations" do
        ...
        expect(new_scope_limitation.count).to eq 4 # keys in the object hash
      end
    end
  end
